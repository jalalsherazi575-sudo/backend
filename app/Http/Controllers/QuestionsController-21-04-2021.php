...
// print_r($questionType);exit();
		 ...
		 if ($checkduplicate==0) {
			 ...
			 if($request->hasFile('audio')) {
				 ...
				 if($check) {
				   $destinationPath = 'questionAudios';
				   $file->move($destinationPath,$filename);
			 ...
			 if ($isNotValidaudio==1) {
                   flash()->error('Please upload valid Question Audio File.');
			       return redirect()->to('/admin/questions/add');
			 }
			 ...
			 if ($questionType==1) {
				...
				foreach ($questionAsk as $key => $value) {
					...
					if ($request->hasFile('questionImage')) {
					     ...
					     if (isset($request->file('questionImage')[$key])) {

					   	   $files = $request->file('questionImage')[$key];
					...
					if ($request->hasFile('questionAudio')) {
					 
					     if (isset($request->file('questionAudio')[$key])) {
					           
					           $files = $request->file('questionAudio')[$key];

					   	       $optionImage = time().$files->getClientOriginalName();
					           $extension = $files->getClientOriginalExtension();

					           $destinationPath = 'questionAudioSelection';
	   ...
	   public function deleteAudioSelect($id) {
	   	$delete=DB::delete("update tblquestionimage SET questionAudio='' where id=$id");
	   	echo 1;
	   	exit();
	   }
		          ...
		          if($request->hasFile('questionImage')) {
                     ...
                     if($request->hasFile('questionAudio')) {
					  ...
					  // $allowedfileExtension=['pdf','jpg','png','docx','jpeg','doc','gif'];
					   ...
					   $files = $request->file('questionAudio');
					    ...
					    foreach($files as $key => $file) {
                         ...
                         //$check=in_array($extension,$allowedfileExtension);
							...
							// if($check) {
	                           ...
	                           $getLatestCreatedQues = DB::table('tblquestionimage')->where('questionId',$id)->get();

	                           // print_r($getLatestCreatedQues)

	                           foreach ($getLatestCreatedQues as $key1 => $value1) {
	                           		

	                           		// echo $key;exit();
	                           		DB::table('tblquestionimage')->where('id',$key)->update(['questionAudio'=>$filename1]);
