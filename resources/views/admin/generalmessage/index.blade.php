	...
	<script>
		jQuery(document).ready(function () {
			var table = jQuery('#users-datatable').dataTable({
				...
				"language": {
		      "emptyTable": "No record found of general messages."
		         },
		...
		function check_delete(id)
		{
			...
			$.ajax({
		        url: "{{URL::to('admin/')}}/generalmessage/delete/"+id,
		        type: "get",
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="{{URL::to('admin/generalmessage/')}}">Message</a></li>
                
            </ol>
            <div class="page-actions">
            	@if(checkPermission(Auth::user()->id,'create',$currentMenuId))
                <a href="{{URL::to('admin/generalmessage/add')}}" class="btn btn-primary"><i class="icon-fa icon-fa-plus"></i> New Message</a>
                @endif
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <div class="card-body">
					    <form method="post" action="{{ URL::to('admin/generalmessage/deleteall')}}" name="form">
							<input type="hidden" name="_token" value="{{{ csrf_token() }}}" />
	                        <input type="hidden" name="delId" id = "delId" value="" />
							<table id="users-datatable" class="table table-striped table-bordered" cellspacing="0" width="100%">
	                            <thead>
		                            <tr>
										<th width="15%">Title Key</th>
		                                @if ($language)
											@foreach($language as $val)
												<th>Message </th>
										    @endforeach
										@endif	
		                                <th width="13%">Is App Message</th>
	                            ...
	                            @foreach($messages as $vals)
		                            <tr>
		                                ...
		                                @if($language)
		                                 	@foreach($language as $val)
									      		<td>{{ Laraspace\Http\Controllers\CommanController::getMessageValue($vals['id'],$val->id) }}</td>
									      	@endforeach
										@endif
										@if($vals['is_app_msg']==1)
											<td>Yes</td>
									    @else 
		                                	<td>No</td>
									    @endif
										<td>
											@if(checkPermission(Auth::user()->id,'update',$currentMenuId))
												<a href="{{URL::to('admin/generalmessage/edit/'.$vals['id'].'')}}" class="btn btn-default btn-sm"><i class="icon-fa icon-fa-search"></i>Edit</a>
											@endif
										</td>
									</tr>
	                            @endforeach
