...
<script type="text/javascript">
    ...
    function addNewOptions() {
        ...
        $('#secondansweroptionslist').append('<div  id="optionid'+optioncount+'"><label>Option '+optioncount+'</label><div class="row"><div class="col-sm-3 col-md-3 col-lg-3"><label>Text</label><br>  <input type="text" style="width:90%;" name="questionImageText['+optioncount+']"  maxlength="100"  id="text'+optioncount+'" class="form-control" placeholder="Melon" value=""></div><div class="col-sm-2 col-md-2 col-lg-2"><label>Is Correct Answer</label><br><input type="radio" style="width:40%;" name="isCorrectAnswer['+optioncount+']"    id="isCorrectAnswer'+optioncount+'" class="form-control radioinstant" onclick="return checkRadio('+optioncount+');" value="1"></div><span onclick="return removeSelection('+optioncount+');" class="delete_btn">Delete</span></div></div>');
        $('#optioncount').val(optioncount);
    }
    $(document).ready(function() {
    // Cache the original options for subject and topic dropdowns
    var originalSubjectOptions = $("#subjectId").html();
    var originalTopicOptions = $("#topicId").html();

    // Function to populate subject dropdown based on the selected category
    function populateSubjects(categoryId, selectedSubjectId) {
        $.ajax({
            url: "{{URL::to('admin/')}}/questions/getSubject/" + categoryId,
            type: "get",
            success: function(html) {
                // Reset subject dropdown to original options
                $("#subjectId").html(originalSubjectOptions);
                // Append new options to the subject dropdown
    ...
    // Function to populate topic dropdown based on the selected subject
    function populateTopics(subjectId, selectedTopicIds) {
        $.ajax({
            url: "{{URL::to('admin/')}}/questions/getTopic/" + subjectId,
            type: "get",
            success: function(html) {
               // Reset topic dropdown to original options
            $("#topicId").html(originalTopicOptions);
            // Append new options to the topic dropdown
            $.each(html, function(key, value) {
                var selected = selectedTopicIds.includes(value.id) ? 'selected' : '';
                $("#topicId").append('<option value="' + value.id + '" ' + selected + '>' + value.topicName + '</option>');
            });
            }
        });
    }
    ...
    // Pre-select category if available
            var selectedCategoryId = {{ $questions->categoryId ?? '' }};
           
    $("#categoryId").val(selectedCategoryId);

    // On page load, populate subjects based on the selected category
    populateSubjects(selectedCategoryId, {{ $questions->subjectId ?? '' }});
        // On subject change, populate topics based on the selected subject
        $("#subjectId").change(function() {
            var subjectId = $(this).val();
            var selectedTopicIds = {!! json_encode($queTopic ?? []) !!}; // Ensure it's an array or object
            
            if (subjectId != "") {
                populateTopics(subjectId, selectedTopicIds);
            } else {
                // If no subject is selected, reset the topic dropdown
                $("#topicId").html('<option value="">Please select Topic</option>');
            }
        });
});
                    ...
                    <form id="validateForm" enctype="multipart/form-data" method="post" action="@if(isset($questions)){{ URL::to('admin/questions/edit/'.$questions->questionId.'')}}@else{{ URL::to('admin/questions/add') }}@endif" name="questions" novalidate>
                        {{csrf_field()}}
                            ...
                            <div class="form-group">
                                ...
                                <select name="categoryId" id="categoryId" required="required" class="form-control ls-select2">
                                    <option value="" disabled selected>Please select a category</option>
                                    @foreach($cat as $value)
                                    <option value="{{ $value->levelId}}" @if(isset($questions) && $questions->categoryId == $value->levelId) selected @endif>{{ $value->levelName }}</option>

                                    @endforeach
                                    <!-- Populate categories based on the selected subject using JavaScript -->
                                    <!-- Example: <option value="1">Category 1</option> -->
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="subjectId" id="subjectId" required="required" class="form-control ls-select2">
                                    <option value="" disabled selected>Please select a subject</option>
                                    <!-- Populate subjects here -->
                                    <!-- Example: <option value="1">Subject 1</option> -->
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="topicId[]" id="topicId" required="required" class="form-control ls-select2" multiple>
                                    <option value="" disabled>Please select topic</option>
                                    <!-- Populate topics based on the selected category using JavaScript -->
                                    <!-- Example: <option value="1">Topic 1</option> -->
                                </select>
                            </div>
                       <!--  <div class="form-group">
                            <label>Select Topic<span class="req">*</span></label>
                        ...
                        <div class="form-group" id="ques_name" @if((isset($questions) && $questions->questionType!=5) || !isset($questions))  @else style="display:none;"  @endif>
                            ...
                            <input type="text" maxlength="250" @if((isset($questions) && $questions->questionType!=5))  @else required="required" @endif class="form-control" value="@if(isset($questions)){{$questions->question}}@endif" name="question" id="question">
                        </div>
                        <div class="form-group">
                            <label>Description<span class="req">*</span></label>
                            <textarea name="description" id="description" required="required" maxlength="300" class="form-control" rows="5" cols="5">@if(isset($questions)){{$questions->description}}@endif</textarea>
                        </div>
                        <div class="form-group">
                            <label>Status</label>
                                    ...
                                    <div id="optionid1">
                                        ...
                                        <div class="row">
                                            <div class="col-sm-3 col-md-3 col-lg-3">
                                                <label>Text</label>
                                                <br>  
                                                <input type="text" style="width:90%;" name="questionImageText[{{$optlist->id}}]"  maxlength="100"   id="text1" class="form-control" placeholder="Melon" value="{{$optlist->questionImageText}}">
                                            </div>
                                ...
                                <div id="optionid{{$k}}">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-3 col-md-3 col-lg-3">
                                            <label>Text</label>
                                            <br>  
                                            <input type="text" style="width:90%;" name="questionImageText[{{$optlist->id}}]"  maxlength="100"  id="text{{$k}}" class="form-control" placeholder="Melon" value="{{$optlist->questionImageText}}">
                                        </div>
                                        ...
                                        <div id="optionid1">
                                            ...
                                            <div class="row">
                                                <div class="col-sm-3 col-md-3 col-lg-3">
                                                    <label>Text</label>
                                                    <br>  
                                                    <input type="text" style="width:90%;" name="questionImageText[1]" @if(!isset($questions))  @endif maxlength="100"   id="text1" class="form-control" placeholder="Melon" value="">
                                                </div>
                                     ...
                                     <label>Option 1</label>  
                                          <div class="row">
                                             ...
                                             <div class="col-sm-3 col-md-3 col-lg-3">
                                              <label>Text</label>
                                              <br>  
                                              <input type="text" style="width:90%;" name="wordText[{{$optlist->id}}]" required="required" maxlength="100"   id="wordText1" class="form-control type2" placeholder="" value="{{$optlist->wordText}}">
                                             </div>
                                     ...
                                     <label>Option {{$k}}</label>  
                                          <div class="row">
                                             ...
                                             <div class="col-sm-3 col-md-3 col-lg-3">
                                              <label>Text</label>
                                              <br>  
                                              <input type="text" style="width:90%;" name="wordText[{{$optlist->id}}]" required="required" maxlength="100"   id="wordText{{$k}}" class="form-control type2" placeholder="Melon" value="{{$optlist->wordText}}">
                                             </div>
                               ...
                               <label>Option 1</label>  
                                    <div class="row">
                                       <div class="col-sm-3 col-md-3 col-lg-3">
                                        <label>Text</label>
                                        <br>  
                                        <input type="text" style="width:90%;" name="wordText[1]" @if(!isset($questions)) required="required" @endif maxlength="100" id="wordText1" class="form-control type2" placeholder="" value="">
                                       </div>
                                       <div class="col-sm-3 col-md-3 col-lg-3">
                                        <label>Sequence</label>
                                        <br>  
                                         ...
                                         <label>Option 1</label>  
                                              <div class="row">
                                                 ...
                                                 <div class="col-sm-6 col-md-6 col-lg-6">
                                                  <label>Text</label>
                                                  <br>  
                                                  <input type="text" style="width:90%;" name="wordTextTwo[{{$optlist->id}}]" required="required" maxlength="100"   id="wordTextTwo1" class="form-control type3" placeholder="" value="{{$optlist->wordText}}">
                                                 </div>
                                         ...
                                         <label>Option {{$k}}</label>  
                                              <div class="row">
                                                 ...
                                                 <div class="col-sm-6 col-md-6 col-lg-6">
                                                  <label>Text</label>
                                                  <br>  
                                                  <input type="text" style="width:90%;" name="wordTextTwo[{{$optlist->id}}]" required="required" maxlength="100"   id="wordTextTwo{{$k}}" class="form-control type3" placeholder="Melon" value="{{$optlist->wordText}}">
                                                 </div>
                                   ...
                                   <label>Option 1</label>  
                                        <div class="row">
                                           <div class="col-sm-6 col-md-6 col-lg-6">
                                            <label>Text</label>
                                            <br>  
                                            <input type="text" style="width:90%;" name="wordTextTwo[1]" @if(!isset($questions)) required="required" @endif maxlength="100" id="wordTextTwo1" class="form-control type3" placeholder="" value="">
                                           </div>
                                           <div class="col-sm-4 col-md-4 col-lg-4">
                                            <label>Sequence</label>
                                            <br>  
                                     ...
                                     <label>Option 1</label>  
                                          <div class="row">
                                             ...
                                             <div class="col-sm-3 col-md-3 col-lg-3">
                                              <label>Text</label>
                                              <br>  
                                              <input type="text" style="width:90%;" name="optionName[{{$optlist->id}}]"  maxlength="100"   id="wordText1" class="form-control type4" placeholder="" value="{{$optlist->optionName}}">
                                             </div>
                                     ...
                                     <label>Option {{$k}}</label>  
                                          <div class="row">
                                             ...
                                             <div class="col-sm-3 col-md-3 col-lg-3">
                                              <label>Text</label>
                                              <br>  
                                              <input type="text" style="width:90%;" name="optionName[{{$optlist->id}}]" required="required" maxlength="100"   id="optionName{{$k}}" class="form-control type4" placeholder="Melon" value="{{$optlist->optionName}}">
                                             </div>
                               ...
                               <label>Option 1</label>  
                                    <div class="row">
                                       <div class="col-sm-3 col-md-3 col-lg-3">
                                        <label>Text</label>
                                        <br>  
                                        <input type="text" style="width:90%;" name="optionName[1]" @if(!isset($questions)) required="required" @endif maxlength="100" id="optionName1" class="form-control type4" placeholder="" value="">
                                       </div>
                                     ...
                                     <label>Option 1</label>  
                                          <div class="row">
                                             ...
                                             <div class="col-sm-2 col-md-2 col-lg-2">
                                              <label>Text</label>
                                              <br>  
                                              <input type="text"  name="optionNameAl[{{$optlist->id}}]"  maxlength="100"   id="optionNameAl1" class="form-control" placeholder="" value="{{$optlist->optionName}}">
                                             </div>
                                     ...
                                     <label>Option {{$k}}</label>  
                                          <div class="row">
                                             ...
                                             <div class="col-sm-2 col-md-2 col-lg-2">
                                              <label>Text</label>
                                              <br>  
                                              <input type="text"  name="optionNameAl[{{$optlist->id}}]"  maxlength="100"   id="optionNameAl{{$k}}" class="form-control" placeholder="" value="{{$optlist->optionName}}">
                                             </div>
                               ...
                               <label>Option 1</label>  
                                    <div class="row">
                                       ...
                                       <div class="col-sm-2 col-md-2 col-lg-2">
                                        <label>Text</label>
                                        <br>  
                                        <input type="text"  name="optionNameAl[1]"   maxlength="100"   id="optionNameAl1" class="form-control" placeholder="" value="">
                                       </div>
